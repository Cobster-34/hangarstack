...
export function HangarCanvas({ onMovePlacement, onRemovePlacement, onUpdateStatus, onUpdateDispatchOrder }: HangarCanvasProps) {
  ...
  const [contextMenu, setContextMenu] = useState<{ placement: AircraftPlacement; x: number; y: number } | null>(null)
  ...
  // Native DOM mouse events on the container — avoids all Konva event bubbling issues
  useEffect(() => {
    ...
    const onMouseMove = (e: MouseEvent) => {
      ...
      if (mode === 'panning') {
        ...
        const newTransform = { ...t, x: panStart.current.tx + dx, y: panStart.current.ty + dy }
      ...
      if (mode === 'rotating' && rotateState.current) {
        const rs = rotateState.current
        const placement = placementsRef.current.find(p => p.id === rs.placementId)
        if (!placement) return
        const rect = el.getBoundingClientRect()
        const acScreenX = ftToPx(placement.x_ft) * t.scale + t.x + rect.left
        const acScreenY = ftToPx(placement.y_ft) * t.scale + t.y + rect.top
        const currentAngle = Math.atan2(e.clientY - acScreenY, e.clientX - acScreenX) * 180 / Math.PI
        let newRot = rs.startRot + (currentAngle - rs.startAngle)
        if (e.shiftKey) newRot = Math.round(newRot / 15) * 15
        dispatch({ type: 'UPDATE_PLACEMENT', placement: { ...placement, rotation_deg: newRot } })
        return
      }
    }
    ...
    const onMouseUp = (e: MouseEvent) => {
      ...
      if (mode === 'rotating' && rotateState.current) {
        const placement = placementsRef.current.find(p => p.id === rotateState.current!.placementId)
  ...
  const handleBackgroundMouseDown = (e: Konva.KonvaEventObject<MouseEvent>) => {
    modeRef.current = 'panning'
    panStart.current = { mouseX: e.evt.clientX, mouseY: e.evt.clientY, tx: transform.x, ty: transform.y }
  }

  // Aircraft mousedown = drag
  ...
  // Rotation handle mousedown = rotate
  const handleRotateMouseDown = (e: Konva.KonvaEventObject<MouseEvent>, p: AircraftPlacement) => {
    e.cancelBubble = true
    modeRef.current = 'rotating'
    setRotatingId(p.id)
    const rect = containerRef.current!.getBoundingClientRect()
    const acScreenX = ftToPx(p.x_ft) * transform.scale + transform.x + rect.left
    const acScreenY = ftToPx(p.y_ft) * transform.scale + transform.y + rect.top
    const startAngle = Math.atan2(e.evt.clientY - acScreenY, e.evt.clientX - acScreenX) * 180 / Math.PI
    rotateState.current = { placementId: p.id, startAngle, startRot: p.rotation_deg }
  }
  ...
  for (let x = 0; x <= activeHangar.width_ft; x += GRID_FT) {
    gridLines.push(
      <Line key={`gx${x}`} points={[ftToPx(x), 0, ftToPx(x), D]}
  ...
  for (let y = 0; y <= activeHangar.depth_ft; y += GRID_FT) {
    gridLines.push(
      <Line key={`gy${y}`} points={[0, ftToPx(y), W, ftToPx(y)]}
        stroke={y % 50 === 0 ? 'rgba(255,255,255,0.08)' : 'rgba(255,255,255,0.03)'}
        strokeWidth={y % 50 === 0 ? 1 : 0.5} />
    )
  }

  const violationIds = new Set(state.violations.flatMap(v => [v.placement_id_a, v.placement_id_b ?? '']))
  const nextDispatchOrder = Math.max(0, ...state.placements.map(p => p.dispatch_order ?? 0)) + 1

  return (
    <div
      ref={(node) => { (containerRef as any).current = node; containerCallback(node) }}
      style={{
        flex: 1, overflow: 'hidden', position: 'relative',
        background: 'var(--bg-base)',
        cursor: modeRef.current === 'rotating' ? 'crosshair' : modeRef.current === 'panning' ? 'grabbing' : 'grab',
        userSelect: 'none',
      }}
    >
      {/* HUD */}
      <div style={{
        ...
        <span style={{ color: 'var(--border-strong)' }}>·</span>
        <span style={{ color: 'var(--text-muted)' }}>scroll=zoom · drag bg=pan · drag plane=move · drag ↻=rotate · ⇧=snap</span>
      </div>
      ...
      >
        ...
        <Layer>
          <Rect x={0} y={0} width={W} height={D}
            ...
            onClick={() => { dispatch({ type: 'SET_SELECTION', selection: { type: null, id: null } }); setContextMenu(null) }}
          />
          {gridLines}
          <Text x={W / 2 - 35} y={-20} text={`${activeHangar.width_ft}′ wide`}
            fontSize={10} fill="rgba(255,255,255,0.22)" fontFamily="DM Mono, monospace" />
          <Text x={-50} y={D / 2} text={`${activeHangar.depth_ft}′`}
            fontSize={10} fill="rgba(255,255,255,0.22)" fontFamily="DM Mono, monospace" rotation={-90} />

          {activeHangar.obstructions.map(obs => (
            <Group key={obs.id} x={ftToPx(obs.x_ft)} y={ftToPx(obs.y_ft)} rotation={obs.rotation_deg}>
              <Rect width={ftToPx(obs.width_ft)} height={ftToPx(obs.height_ft)}
                fill="rgba(251,146,60,0.08)" stroke="rgba(251,146,60,0.35)"
                strokeWidth={1} dash={[4, 3]} cornerRadius={2} />
              <Text x={3} y={3} text={obs.label} fontSize={9} fill="rgba(251,146,60,0.6)" fontFamily="DM Mono, monospace" />
            </Group>
          ))}
          ...
          {activeHangar.doors.map(door => {
            ...
            return (
              <Group key={door.id}>
                <Rect x={x} y={y} width={isHoriz ? doorW : 7} height={isHoriz ? 7 : doorW}
                  fill="rgba(96,165,250,0.2)" stroke="rgba(96,165,250,0.7)" strokeWidth={2} />
                <Text x={x + 2} y={isHoriz ? y - 16 : y - 2} text={`▲ ${door.label}`}
                  fontSize={9} fill="rgba(96,165,250,0.7)" fontFamily="DM Mono, monospace" />
              </Group>
            )
          })}
        </Layer>
        ...
        <Layer>
          {state.placements.map(p => {
            ...
            return (
              <Group key={p.id}
                x={ftToPx(p.x_ft)} y={ftToPx(p.y_ft)} rotation={p.rotation_deg}
              ...
              >
                ...
                {sil.propDiscs.map((disc, i) => (
                  <Ellipse key={i} x={disc.x} y={disc.y} radiusX={disc.rx} radiusY={disc.ry}
                    fill="transparent" stroke={strokeColor + '55'} strokeWidth={1} dash={[2, 3]} />
                ))}
                <Text text={ac.tail_number} x={-hw + 2} y={-7}
                  fontSize={Math.max(7, Math.min(11, hw / 3.5))}
                  fill={cfg.color} fontFamily="DM Mono, monospace" />
                <Text text={`${ac.manufacturer} ${ac.model}`.substring(0, 16)} x={-hw + 2} y={6}
                ...
                {p.dispatch_order !== null && (
                  <Group x={hw - 16} y={-hl + 4}>
                    <Rect x={0} y={0} width={14} height={14} fill="rgba(250,204,21,0.92)" cornerRadius={3} />
                    <Text text={String(p.dispatch_order)} x={0} y={1} width={14} align="center"
                      fontSize={9} fill="#000" fontFamily="DM Mono, monospace" fontStyle="bold" />
                  </Group>
                )}
                {isSelected && (
                  <Rect x={-hw - 5} y={-hl - 5} width={hw * 2 + 10} height={hl * 2 + 10}
                    fill="transparent" stroke="rgba(96,165,250,0.45)"
                    strokeWidth={1.5} dash={[4, 3]} cornerRadius={4} />
                )}
                {isSelected && (
                  <Group>
                    <Line points={[0, -hl, 0, -handleDist + 7]}
                      stroke="rgba(96,165,250,0.35)" strokeWidth={1} dash={[3, 3]} />
                    <Ellipse x={0} y={-handleDist} radiusX={7} radiusY={7}
                      fill="rgba(96,165,250,0.12)" stroke="rgba(96,165,250,0.8)" strokeWidth={1.5}
                      onMouseDown={(e) => handleRotateMouseDown(e, p)} />
                    <Text x={-4} y={-handleDist - 5} text="↻" fontSize={9}
                      fill="rgba(96,165,250,0.85)" fontFamily="DM Mono, monospace" />
                    <Text x={10} y={-handleDist - 4}
                      text={`${Math.round(((p.rotation_deg % 360) + 360) % 360)}°`}
                      fontSize={8} fill="rgba(96,165,250,0.65)" fontFamily="DM Mono, monospace" />
                  </Group>
                )}
              </Group>
            )
